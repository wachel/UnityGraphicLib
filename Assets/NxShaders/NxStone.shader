Shader "Unlit/NxStone"
{
	Properties
	{
		...
		[Toggle(XRAY_EX_ENABLE)] XRAY_EX_ENABLE("XRAY_EX_ENABLE", float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature XRAY_EX_ENABLE
			...
			#include "NxStoneVS.cginc"
			#include "NxStonePS.cginc"
			...
			PS_INPUT vert (appdata v)
			{
				...
				//LightMapTransform = float4(unity_LightmapST.zw, 0, 0);
				//lightmap_scale = 
